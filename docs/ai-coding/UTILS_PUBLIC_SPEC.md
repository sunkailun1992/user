...
- 任何先查询后修改的操作，都必须把查询得到的版本号写入修改对象的 `version` 字段。
- 更新逻辑不能绕过 `version` 直接覆盖数据。
- AI 生成 Service 更新方法时，必须检查入参或查询对象是否携带 `version`。
- 如果业务需要先加载实体再更新，更新前要保留数据库当前版本号，并在提交更新时交给框架校验。
...
- 重复请求使用 `A0506 | 用户重复请求`。
- 版本不匹配使用 `A0805 | 用户 API 请求版本不匹配` 或项目已有版本错误枚举。
