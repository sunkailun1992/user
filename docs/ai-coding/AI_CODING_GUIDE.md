...
7. 判断本次代码是否属于公共工具类、通用组件、基础配置或跨微服务复用能力；如果是，先切到同级 `utils` 项目实现并安装依赖。
8. 只有明确属于当前微服务业务边界的代码，才按当前项目现有结构创建 Java 文件。
9. 代码编写完成后检查项目根目录 `README.md`，已有内容时补充本次新增或调整的业务说明，缺少文件时新建。
10. 编译验证。
...
- 不要把公共工具类、通用组件、基础配置或跨微服务复用能力直接写进业务微服务；这些内容必须放到同级 `utils` 项目维护。
