...
公共工具类、通用组件、基础配置和跨微服务复用能力统一放在同级 `utils` 项目维护，业务微服务只通过依赖引用。
...
- 确认属于公共能力时，先修改同级 `utils` 项目，执行 `mvn -q -DskipTests install`，再回到当前微服务升级或确认 `com:utils` 依赖版本。
...
- 是否把公共工具类、通用组件、基础配置或跨微服务复用能力放到同级 `utils` 项目，而不是写进当前业务微服务。
