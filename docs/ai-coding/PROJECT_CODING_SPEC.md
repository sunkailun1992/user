...
AI 编程产出的代码、SQL、配置、脚本、测试、前端调用示例等内容都必须保持逐行注释：
...
- AI 新增或修改的每一行编程内容都必须补充注释，说明该行内容的用途、功能、业务含义或框架衔接原因。
- 包声明、import、空行、单独的大括号和 Markdown 普通说明段落可以不写无意义注释；其余新增或修改的代码、SQL、配置、脚本、测试和示例默认都要逐行注释。
