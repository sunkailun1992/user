...
`user` 与 `message` 是同级独立业务模块，互相调用应通过 API 契约完成；底层公共工具、通用组件、基础配置和跨微服务复用能力统一放在真实同级 `../utils` 项目维护，业务微服务只通过依赖引用。
...
- 公共 `Example*` 模板的唯一源头是真实同级 `../utils/docs/ai-coding/examples`。
- 修改公共模板时，必须先修改真实同级 `../utils` 源头，再同步到 `user`、`message` 等业务微服务本地副本。
...
- 不得在当前项目根目录内创建同级项目副本；如果 IDEA 展示了 `utils`、`message` 等目录，应先检查是否存在误放的嵌套 checkout。
...
AI 编程产出的代码、SQL、配置、脚本、测试、前端调用示例等内容必须先遵守 `AI_COMMENT_STYLE_GUIDE.md`，再按本节落地到 Java 和数据库对象。注释是次优手段，优先通过命名、类型、小方法和清晰分层让代码自解释。
...
- 认证授权、租户、权限、Redis、动态数据源、数据库乐观锁、SQL 参数校验、事务、异常处理、返回值组装等关键逻辑，应在逻辑块前说明“为什么这样做”和“守住什么边界”。
- 注释要解释业务目的、风险边界和框架衔接，不要只翻译代码或重复方法名。
- 禁止凑数、过时、误导性注释；注释与代码行为不一致时必须同步修正。
- 禁止注释掉旧实现、临时 SQL、调试 main 或整块废弃代码；历史版本交给 Git。
- 普通 getter/setter、显而易见的变量赋值、import、空行和单独大括号不需要注释。
- 注释必须维持当前文件缩进、换行、段落和对齐风格；不要把注释集中堆在行尾导致代码难读。
...
- 是否先检查真实同级 `../utils` 项目已有能力，并优先复用已有公共工具以减少当前微服务代码量。
- 是否把确实缺失的公共工具类、通用组件、基础配置或跨微服务复用能力放到真实同级 `../utils` 项目，而不是写进当前业务微服务。
- 是否避免在 `user` 根目录内保留同级项目的嵌套 checkout。
- 是否给受保护接口加 `@PreAuthorize`。
- 是否按 `AI_COMMENT_STYLE_GUIDE.md` 补齐必要类注释、字段注释、方法 JavaDoc 和关键逻辑块注释，并保持排版美观。
