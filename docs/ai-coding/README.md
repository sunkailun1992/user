...
10. 再读 `PROJECT_CODING_SPEC.md`，确认通用分层、返回值、权限、多租户、注释和检查清单。
11. 涉及接口、权限、数据隔离、脱敏、上传下载、SQL、XSS 或测试安全场景时，读 `SECURITY_CODING_SPEC.md`。
12. 涉及错误码、乐观锁、数据库变更、分支流程或公共能力时，读 `UTILS_PUBLIC_SPEC.md`。
13. 当前微服务业务改造时，读项目根目录 `README.md`。
14. 新增业务模块时参考 `examples/`；该目录是从 `utils/docs/ai-coding/examples` 同步的本地副本。
15. 需要追溯原始迁移资料时再看 `archive/`。
16. 涉及公共工具类、通用组件、基础配置、跨微服务复用能力时，先检查真实同级 `../utils` 项目是否已有能力；已有则直接复用，缺失时再到 `../utils` 实现，并在当前微服务升级依赖和调用。
...
- Controller 返回统一使用 `com.kellen.utils.response.ApiResponse`。
...
- 复制到其他微服务时，可以复用本目录下的 `README.md`、`AI_CODING_GUIDE.md`、`AI_DESIGN_PATTERN_GUIDE.md`、`AI_AUTOMATION_WORKFLOW.md`、`AI_ENGINEERING_GUARDRAILS.md`、`PROJECT_CODING_SPEC.md`、`SECURITY_CODING_SPEC.md`、`UTILS_PUBLIC_SPEC.md`、`examples/`，但必须按目标项目语言、架构和边界改写。
