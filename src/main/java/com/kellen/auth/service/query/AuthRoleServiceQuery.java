...
public class AuthRoleServiceQuery {
    ...
    public QueryWrapper<AuthRole> query(AuthRoleQuery authRoleQuery, QueryWrapper<AuthRole> queryWrapper) {
        ...
        // 计算排序字段，未传时按排序字段查询。
        String sortField = StringUtils.defaultIfBlank(authRoleQuery.getCollationFields(), DEFAULT_SORT_FIELD);
        // 升序标识为 true 时按升序排序。
        ...
        if (StringUtils.isNotBlank(authRoleQuery.getFields())) {
            // 拼接指定查询列。
            queryWrapper.select(authRoleQuery.getFields());
        }
        // 返回完整包装器。
        return queryWrapper;
    }
